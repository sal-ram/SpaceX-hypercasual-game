...
public class BotController : Controller
{
    ...
    private void GoingToStartofLadder()
    {
        ...
        var direction = _positionOfFirstStep - transform.position;
        agent.velocity = direction.normalized * speed;
        IsPatrolling = true;
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag(_brickCollector.brickTag))
        {
            ...
            if (_brickCollector.GetAmountOfBricks() > 12)
            {
                GoingToLadder();
            }
            else
        ...
        {
            var destroyHandler = other.gameObject.GetComponent<DestroyHandler>();
