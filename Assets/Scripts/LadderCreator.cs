...
public class LadderCreator : MonoBehaviour
{
    ...
    void BuildLadder()
    {
        for (int i = 0; i < _lenghthOfLadder - 1; i++)
        {
            ...
            if (i > 0)
            {
                ...
                //prevStep.GetComponentInChildren<CollisionHandlerStepTest>().linkToNextBarrier = step.transform;
            }
            prevStep = step;
        }

        var endPointStart = Instantiate(_endOfLadderPoint, transform);
        endPointStart.transform.position = prevStep.transform.position + new Vector3(0, 0, 3f);
        prevStep.GetComponentInChildren<CollisionHandlerStep>().linkToNextBarrier = _nexLevelPoint.transform;
        OnLadderIsBuilded.Invoke();
        //prevStep.GetComponentInChildren<CollisionHandlerStepTest>().linkToNextBarrier = _nexLevelPoint.transform;
    }
    ...
    public void HideLadder()
    {
        ...
        {
            //_listOfSteps[i].SetActive(false);
